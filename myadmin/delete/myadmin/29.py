...
def add_post(request):
    if request.method == 'POST' or request.is_ajax():
        ...
        Post_master.objects.create(post_id=id+1,post_code=postcode,post_desc=postname,department_code_id=dept_id)
...
def save_designation(request):
    if request.method == 'POST':
        ...
        if Level_Desig.objects.filter(designation=designation).exists():
            ...
            Level_Desig.objects.create(user_role=role,designation_code=id+1,department=department,designation=designation,pc7_levelmax=pc7_levelmax,pc7_levelmin=pc7_levelmin,department_code_id=dept_id,d_level=d_level,contactnumber=contact,official_email_ID=email,head_flag=head)
            messages.success(request, 'Data Saved')
