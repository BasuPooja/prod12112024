...
def add_designation(request):
    # try:
        ...
        post=Post_master.objects.filter(Q(delete_flag=True) & ~Q(post_code=None)).values('post_code').order_by('post_code').distinct('post_code')
...
def cal_desig(request):
    # try:
        if request.method == "GET":
            ...
            # id_depart=Post_master.objects.filter(post_desc=post).values('department_code_id')[0]['department_code_id']
            # depart=departMast.objects.filter(department_code=id_depart).values('department_name')[0]['department_name']
...
def add_post(request):
    try:
        if request.method == 'POST' or request.is_ajax():
            ...
            Post_master.objects.create(post_id=id+1,post_code=postcode,post_desc=postname,department_code_id=dept_id,category=category,pc7_levelmax=pc7_levelmax,pc7_levelmin=pc7_levelmin)
...
def save_designation(request):
    # try:
        if request.method == 'POST':
            ...
            # pc7_levelmax = request.POST.get('pc7_levelmax')
            # pc7_levelmin = request.POST.get('pc7_levelmin')
            # d_level = request.POST.get('d_level')
            ...
            # add role,category , paylevels while adding designation from post

            # role=category.objects.filter(category=categoryy).values('user_role')[0]['user_role']
            # if post != '':
            if post != '':
                ...
                paylevel_min=None
                dept_id=None
            print(category,paylevel_max,paylevel_min,dept_id)
            
            if email == '':
                email=None
            if contact == '':
                contact=None
            # if departMast.objects.filter(department_name=department).exists():
            #     dept_id=departMast.objects.filter(department_name=department)[0].department_code
            # else:
            #     dept_id=None
            if Level_Desig.objects.all().exists():
            ...
            if Level_Desig.objects.filter(designation=designation).exists():
                messages.error(request, 'Designation already present')
            elif Level_Desig.objects.filter(official_email_ID=email, official_email_ID__isnull=False).exists():
                messages.error(request, 'Email Id already present')
                ...
                Level_Desig.objects.create(pc7_levelmax=paylevel_max,pc7_levelmin=paylevel_min,d_level=category,designation_code=id+1,department=department,designation=designation,department_code_id=dept_id,contactnumber=contact,official_email_ID=email)
                messages.success(request, 'Data Saved')
            else:
                Level_Desig.objects.create(pc7_levelmax=paylevel_max,pc7_levelmin=paylevel_min,d_level=category,designation_code=id+1,department=department,designation=designation,department_code_id=dept_id,contactnumber=contact,official_email_ID=email,head_flag=head)
                messages.success(request, 'Data Saved')
...
#         type=railwayLocationMaster.objects.filter(Q(location_type_desc='INSTITUTE')|Q(location_type_desc='HEAD QUATER')|Q(location_type_desc='PRODUCTION UNIT')|Q(location_type_desc='OFFICE') & ~Q(location_type=None)).values('location_type').distinct()
